...
if($is_admin) {
    ...
    if($row = mysqli_fetch_assoc($section_result)) {
        ...
        if(strtolower($teacher_section) == 'emilio_jacinto') {
            $has_access = true;
        }
    }
    mysqli_stmt_close($stmt);
...
$students_query = "SELECT lrn_number, CONCAT(last_name, ', ', first_name, ' ', LEFT(middle_name, 1), '.') as full_name, gender FROM tblgrade4emilio_jacinto ORDER BY last_name, first_name";
...
if(isset($_POST['action']) && $_POST['action'] == 'save' && $has_access) {
    ...
    $check_query = "SELECT id FROM tblgrade4emilio_jacintouniform WHERE lrn = ?";
    ...
    if(mysqli_num_rows($check_result) > 0) {
        ...
        $update_query = "UPDATE tblgrade4emilio_jacintouniform SET student_name=?, gender=?, pe_tshirt=?, pe_pants=?, socks=?, shoes=?, school_uniform=?, uniform_bottom=?, created_by=? WHERE lrn=?";
        ...
        if(mysqli_stmt_execute($stmt)) {
            ...
            logAction($conn, 'EDIT', 'Grade 1 - Emilio_Jacinto (Uniform)', $student_name, 'Updated uniform record', $created_by);
        } else {
            $error_message = "Error updating record: " . mysqli_error($conn);
        }
    } else {
        // Insert new record
        $insert_query = "INSERT INTO tblgrade4emilio_jacintouniform (lrn, student_name, gender, pe_tshirt, pe_pants, socks, shoes, school_uniform, uniform_bottom, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if(mysqli_stmt_execute($stmt)) {
            ...
            logAction($conn, 'ADD', 'Grade 4 - Emilio_Jacinto (Uniform)', $student_name, 'Added new uniform record', $created_by);
        } else {
...
if(isset($_GET['delete']) && $has_access) {
    ...
    $get_name_query = "SELECT student_name FROM tblgrade4emilio_jacintouniform WHERE id = ?";
    ...
    // Delete the record
    $delete_query = "DELETE FROM tblgrade4emilio_jacintouniform WHERE id = ?";
    $stmt = mysqli_prepare($conn, $delete_query);
    mysqli_stmt_bind_param($stmt, "i", $id);
    if(mysqli_stmt_execute($stmt)) {
        $success_message = "Record deleted successfully!";
        logAction($conn, 'DELETE', 'Grade 4 - Emilio_Jacinto (Uniform)', $student_name, 'Deleted uniform record', $_SESSION['username']);
    } else {
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $query = "SELECT * FROM tblgrade4emilio_jacintouniform WHERE lrn LIKE '%$search%' OR student_name LIKE '%$search%' ORDER BY student_name";
} else {
    $query = "SELECT * FROM tblgrade4emilio_jacintouniform ORDER BY student_name";
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="action-buttons-top no-print">
            ...
            <button class="supplies-btn" onclick="location.href='emilio_jacinto_supplies.php'">
            ...
            <div class="page-header-content">
                <div>
                    <h1 class="page-title">
                        ...
                        Grade 4 - Emilio_Jacinto Uniforms
                    </h1>
        ...
        <div class="search-section no-print">
            <form method="GET" action="">
                <div class="search-container">
                    ...
                    <?php if(!empty($search)): ?>
                        <button type="button" class="btn-clear" onclick="location.href='emilio_jacinto_uniform.php'">
