...
if($is_admin) {
    ...
    if($row = mysqli_fetch_assoc($section_result)) {
        ...
        if(strtolower($teacher_section) == 'tsiko') {
            $has_access = true;
        }
    }
    mysqli_stmt_close($stmt);
    ...
    if(!$validation_error) {
        ...
        if(mysqli_num_rows($check_result) > 0) {
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'EDIT', 'Grade 1 - Tsiko (Uniform)', $student_name, 'Updated uniform record', $created_by);
            } else {
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'ADD', 'Grade 1 - Tsiko (Uniform)', $student_name, 'Added new uniform record', $created_by);
            } else {
...
if(isset($_GET['delete']) && $has_access) {
    ...
    if(mysqli_stmt_execute($stmt)) {
        ...
        logAction($conn, 'DELETE', 'Grade 1 - Tsiko (Uniform)', $student_name, 'Deleted uniform record', $_SESSION['username']);
