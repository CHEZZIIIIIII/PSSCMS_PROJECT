...
if($is_admin) {
    ...
    if($row = mysqli_fetch_assoc($section_result)) {
        if(strtolower($row['section']) == 'macapagal') $has_access = true;
    }
    mysqli_stmt_close($stmt);
}

$students_query = "SELECT lrn_number, CONCAT(last_name, ', ', first_name, ' ', LEFT(middle_name, 1), '.') as full_name, gender FROM tblgrade5macapagal ORDER BY last_name, first_name";
$students_result = mysqli_query($conn, $students_query);

$existing_lrns = [];
$existing_result = mysqli_query($conn, "SELECT lrn FROM tblgrade5macapagaluniform");
...
if(isset($_POST['action']) && $_POST['action'] == 'save' && $has_access) {
    ...
    if(!$validation_error) {
        $check_query = "SELECT id FROM tblgrade5macapagaluniform WHERE lrn = ?";
        ...
        if(mysqli_num_rows($check_result) > 0) {
            $update_query = "UPDATE tblgrade5macapagaluniform SET student_name=?, gender=?, pe_tshirt=?, pe_pants=?, socks=?, shoes=?, school_uniform=?, uniform_bottom=?, created_by=? WHERE lrn=?";
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'EDIT', 'Grade 5 - Macapagal (Uniform)', $student_name, 'Updated uniform record', $created_by);
            } else {
                $error_message = "Error updating record: " . mysqli_error($conn);
            }
        } else {
            $insert_query = "INSERT INTO tblgrade5macapagaluniform (lrn, student_name, gender, pe_tshirt, pe_pants, socks, shoes, school_uniform, uniform_bottom, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'ADD', 'Grade 5 - Macapagal (Uniform)', $student_name, 'Added new uniform record', $created_by);
            } else {
        ...
        $existing_lrns = [];
        $existing_result2 = mysqli_query($conn, "SELECT lrn FROM tblgrade5macapagaluniform");
...
if(isset($_GET['delete']) && $has_access) {
    ...
    $get_name_query = "SELECT student_name FROM tblgrade5macapagaluniform WHERE id = ?";
    $stmt = mysqli_prepare($conn, $get_name_query);
    mysqli_stmt_bind_param($stmt, "i", $id);
    mysqli_stmt_execute($stmt);
    $name_result = mysqli_stmt_get_result($stmt);
    $student_name = '';
    if($row = mysqli_fetch_assoc($name_result)) $student_name = $row['student_name'];
    mysqli_stmt_close($stmt);

    $delete_query = "DELETE FROM tblgrade5macapagaluniform WHERE id = ?";
    $stmt = mysqli_prepare($conn, $delete_query);
    mysqli_stmt_bind_param($stmt, "i", $id);
    if(mysqli_stmt_execute($stmt)) {
        $success_message = "Record deleted successfully!";
        logAction($conn, 'DELETE', 'Grade 5 - Macapagal (Uniform)', $student_name, 'Deleted uniform record', $_SESSION['username']);
        $existing_lrns = [];
        $existing_result3 = mysqli_query($conn, "SELECT lrn FROM tblgrade5macapagaluniform");
        while($erow = mysqli_fetch_assoc($existing_result3)) $existing_lrns[] = $erow['lrn'];
    } else {
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $query = "SELECT * FROM tblgrade5macapagaluniform WHERE lrn LIKE '%$search%' OR student_name LIKE '%$search%' ORDER BY student_name";
} else {
    $query = "SELECT * FROM tblgrade5macapagaluniform ORDER BY student_name";
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="action-buttons-top no-print">
            ...
            <button class="supplies-btn" onclick="location.href='macapagal_supplies.php'">
        ...
        <div class="page-header no-print">
            <div class="page-header-content">
                <div>
                    <h1 class="page-title"><i class="fas fa-tshirt"></i> Grade 5 - Macapagal Uniforms</h1>
                    <p class="page-description">Manage uniform sizes and requirements</p>
                </div>
                <?php if($has_access): ?>
                    <button class="btn-add" onclick="openAddModal()">
                        <i class="fas fa-plus"></i> Add School Uniform
                    </button>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="search-section no-print">
            <form method="GET" action="">
                <div class="search-container">
                    ...
                    <?php if(!empty($search)): ?>
                        <button type="button" class="btn-clear" onclick="location.href='macapagal_uniform.php'">
        ...
        <div class="table-section">
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>LRN</th><th>Student Name</th><th>Gender</th>
                            <th>PE T-Shirt</th><th>PE Pants</th><th>Socks</th>
                            <th>Shoes</th><th>School Uniform</th><th>Uniform Bottom</th>
                                ...
                                <tr>
                                    ...
                                    <td class="no-print">
                                        <div class="action-buttons">
                                            <?php if($has_access): ?>
                                                <button class="btn-edit" onclick='editRecord(<?php echo json_encode($row); ?>)' title="Edit Record"><i class="fas fa-edit"></i></button>
                                                <button class="btn-delete" onclick="deleteRecord(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($row['student_name']); ?>')" title="Delete Record"><i class="fas fa-trash"></i></button>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select id="lrn" name="lrn" required onchange="updateStudentInfo()">
                                ...
                                ?>
                                    <option value="<?php echo htmlspecialchars($lrn_val); ?>"
                        ...
                        <!-- SHOES: type="text" is required for JS validation to work -->
                        <div class="form-group">
