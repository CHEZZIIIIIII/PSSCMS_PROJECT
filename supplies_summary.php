...
// Initialize arrays to store supplies data
$supplies_data = [];
$supply_items = ['notebooks', 'padpaper', 'bag', 'ballpen', 'pencil', 'sharpener', 'crayons', 'eraser', 'ruler', 'scissors', 'glue'];

$grand_totals = array_fill_keys($supply_items, 0);

for ($grade = 1; $grade <= 6; $grade++) {
    $supplies_data[$grade] = [
        'sections' => [],
        'totals' => array_fill_keys($supply_items, 0)
    ];
    
    $like_pattern = 'tblgrade' . $grade . '%supplies';
    ...
    while ($trow = mysqli_fetch_assoc($tables_result)) {
        ...
        $section_name = '';
        if (preg_match('/tblgrade' . $grade . '([a-z_]+)supplies$/i', $table_name, $matches)) {
        ...
        $section_data = [
            'name' => $section_name,
            'student_count' => 0,
            'items' => array_fill_keys($supply_items, 0)
        ];
        
        $student_table = 'tblgrade' . $grade . strtolower(str_replace(' ', '_', $section_name));
        $student_count_result = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM `$student_table`");
        if ($student_count_result) {
            $cnt_row = mysqli_fetch_assoc($student_count_result);
            $section_data['student_count'] = (int)$cnt_row['cnt'];
        }
        
        foreach ($supply_items as $item) {
            $col_check = mysqli_query($conn,
                "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
                 WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '$table_name' AND COLUMN_NAME = '$item'");
            if ($col_check && mysqli_num_rows($col_check) > 0) {
                $item_query = "SELECT COALESCE(SUM(CAST(`$item` AS UNSIGNED)), 0) as total
                               FROM `$table_name`
                               WHERE `$item` IS NOT NULL AND `$item` != '' AND `$item` REGEXP '^[0-9]+$'";
                $item_result = mysqli_query($conn, $item_query);
                if ($item_result) {
                    $item_row = mysqli_fetch_assoc($item_result);
                    $total = (int)$item_row['total'];
                    $section_data['items'][$item] = $total;
                    $supplies_data[$grade]['totals'][$item] += $total;
                    $grand_totals[$item] += $total;
                }
            }
        }
        
        $supplies_data[$grade]['sections'][] = $section_data;
    }
...
$supply_icons = [
    'notebooks' => 'fa-book', 'padpaper' => 'fa-file-alt', 'bag' => 'fa-bag-shopping',
    'ballpen' => 'fa-pen', 'pencil' => 'fa-pencil-alt', 'sharpener' => 'fa-tools',
    'crayons' => 'fa-palette', 'eraser' => 'fa-eraser', 'ruler' => 'fa-ruler',
    'scissors' => 'fa-cut', 'glue' => 'fa-fill-drip'
];

$chart_grade_labels = [];
$grade_bar_colors = ['#0284c7','#16a34a','#ca8a04','#e11d48','#7c3aed','#ea580c'];
foreach ($supplies_data as $grade => $grade_data) {
    if (!empty($grade_data['sections'])) $chart_grade_labels[] = 'Grade ' . $grade;
}
$chart_item_labels = array_map('ucfirst', $supply_items);
$chart_item_values = array_values($grand_totals);
$grade_total_per_item = [];
foreach ($supplies_data as $grade => $grade_data) {
    if (!empty($grade_data['sections'])) $grade_total_per_item[] = array_values($grade_data['totals']);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>School Supplies Summary - Palatiw Elementary</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: linear-gradient(135deg, #854d0e 0%, #a16207 100%); color: white; padding: 24px 48px; box-shadow: 0 4px 20px rgba(133,77,14,0.15); position: sticky; top: 0; z-index: 100; }
        ...
        .back-btn { display: inline-flex; align-items: center; gap: 8px; background: white; color: #854d0e; border: 2px solid #e2e8f0; padding: 10px 20px; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; text-decoration: none; transition: all 0.2s; }
        .back-btn:hover { border-color: #854d0e; box-shadow: 0 2px 8px rgba(133,77,14,0.15); }
        ...
        .btn-uniform:hover { background: #6d28d9; transform: translateY(-2px); }
        ...
        .page-title i { color: #854d0e; }
        .page-description { font-size: 15px; color: #64748b; }

        .info-notice { background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 12px; padding: 12px 18px; margin-bottom: 24px; font-size: 13px; color: #1d4ed8; display: flex; align-items: center; gap: 10px; }

        .summary-banner { background: linear-gradient(135deg, #854d0e 0%, #a16207 100%); color: white; border-radius: 20px; padding: 32px; margin-bottom: 32px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px; }
        ...
        .sections-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 24px; }
        ...
        .supply-items { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
        .supply-item { display: flex; justify-content: space-between; align-items: center; font-size: 13px; padding: 6px 0; }
        .supply-item-label { color: #64748b; display: flex; align-items: center; gap: 6px; }
        .supply-item-label i { font-size: 11px; }
        .supply-item-count { font-weight: 700; color: #1e293b; }

        .grade-totals { background: #fef9c3; border-radius: 12px; padding: 20px; border: 2px solid #fde047; }
        .grade-totals-title { font-size: 14px; font-weight: 700; color: #854d0e; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.5px; }
        .grade-totals-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 12px; }
        .grade-total-item { text-align: center; padding: 10px; background: white; border-radius: 8px; }
        .grade-total-item-count { font-size: 22px; font-weight: 800; color: #854d0e; }
        .grade-total-item-label { font-size: 11px; color: #64748b; margin-top: 4px; text-transform: capitalize; }
        ...
        .grand-totals-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 16px; }
        ...
        .grand-total-label { font-size: 12px; opacity: 0.85; text-transform: capitalize; }
        ...
        .charts-section-title i { color: #854d0e; }
        ...
        .chart-card-title i { color: #854d0e; font-size: 14px; }
        ...
        @media (max-width: 768px) {
            ...
            .supply-items { grid-template-columns: 1fr; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1 class="page-title"><i class="fas fa-boxes"></i> School Supplies Summary</h1>
            <p class="page-description">Overview of school supplies across all grades and sections</p>
        </div>

        <div class="info-notice">
            <i class="fas fa-info-circle"></i>
            Totals reflect the <strong>actual quantity</strong> each student submitted — e.g., a student who brought 2 notebooks is counted as 2.
        </div>

        <div class="summary-banner">
            <?php
            $total_supplies = array_sum($grand_totals);
            $total_sections = 0;
            foreach ($supplies_data as $gd) $total_sections += count($gd['sections']);
            ?>
            <div class="summary-stat"><div class="summary-stat-number"><?php echo number_format($total_supplies); ?></div><div class="summary-stat-label">Total Supply Items</div></div>
            ...
            <div class="summary-stat"><div class="summary-stat-number">11</div><div class="summary-stat-label">Supply Types</div></div>
        </div>

        <?php foreach ($supplies_data as $grade => $grade_data): ?>
            ...
            <div class="sections-grid">
                ...
                <div class="section-card">
                    ...
                    <div class="supply-items">
                        <?php foreach ($supply_items as $item): ?>
                        <div class="supply-item">
                            <span class="supply-item-label"><i class="fas <?php echo $supply_icons[$item]; ?>"></i><?php echo ucfirst($item); ?></span>
                            <span class="supply-item-count"><?php echo number_format($section['items'][$item]); ?></span>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="grade-totals">
                ...
                <div class="grade-totals-grid">
                    ...
                    <div class="grade-total-item">
                        ...
                        <div class="grade-total-item-label"><?php echo $item; ?></div>
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
        ...
        <div class="grand-totals">
            <div class="grand-totals-title"><i class="fas fa-chart-bar"></i> Grand Totals — All Items (All Grades)</div>
            <div class="grand-totals-grid">
                <?php foreach ($supply_items as $item): ?>
                <div class="grand-total-card">
                    <div class="grand-total-icon"><i class="fas <?php echo $supply_icons[$item]; ?>"></i></div>
                    <div class="grand-total-count"><?php echo number_format($grand_totals[$item]); ?></div>
                    <div class="grand-total-label"><?php echo $item; ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="charts-section">
            ...
            <p class="charts-section-desc">Charts generated from the supply data above</p>
            <div class="charts-grid">
                <div class="chart-card">
                    <div class="chart-card-title"><i class="fas fa-bars"></i> Supply Items — Grand Total Quantities</div>
                    <div class="chart-card-desc">Total quantity of each supply item across all grades</div>
                    <div class="chart-wrapper" style="height:340px;"><canvas id="chartItemTotals"></canvas></div>
                </div>
                <div class="chart-card">
                    <div class="chart-card-title"><i class="fas fa-circle-notch"></i> Distribution by Grade</div>
                    <div class="chart-card-desc">Share of total supply quantities per grade level</div>
                    <div class="chart-wrapper" style="height:340px;"><canvas id="chartGradeDist"></canvas></div>
                </div>
                <div class="chart-card full-width">
                    <div class="chart-card-title"><i class="fas fa-chart-bar"></i> Supply Quantities by Grade — Grouped Comparison</div>
                    <div class="chart-card-desc">Side-by-side quantity of each supply item broken down by grade</div>
                    <div class="chart-wrapper" style="height:380px;"><canvas id="chartGrouped"></canvas></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
    const supplyItems   = <?php echo json_encode($chart_item_labels); ?>;
