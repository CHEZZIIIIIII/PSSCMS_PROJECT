...
if($is_admin) {
    ...
    if($row = mysqli_fetch_assoc($section_result)) {
        ...
        if(strtolower($teacher_section) == 'gumamela') {
            $has_access = true;
        }
    }
    mysqli_stmt_close($stmt);
...
$students_query = "SELECT lrn_number, CONCAT(last_name, ', ', first_name, ' ', LEFT(middle_name, 1), '.') as full_name, gender FROM tblgrade2gumamela ORDER BY last_name, first_name";
$students_result = mysqli_query($conn, $students_query);

// Fetch LRNs that already have a uniform record (to disable in Add dropdown)
$existing_lrns = [];
$existing_result = mysqli_query($conn, "SELECT lrn FROM tblgrade2gumamelauniform");
...
if(isset($_POST['action']) && $_POST['action'] == 'save' && $has_access) {
    ...
    if(!$validation_error) {
        ...
        $check_query = "SELECT id FROM tblgrade2gumamelauniform WHERE lrn = ?";
        ...
        if(mysqli_num_rows($check_result) > 0) {
            $update_query = "UPDATE tblgrade2gumamelauniform SET student_name=?, gender=?, pe_tshirt=?, pe_pants=?, socks=?, shoes=?, school_uniform=?, uniform_bottom=?, created_by=? WHERE lrn=?";
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'EDIT', 'Grade 2 - Gumamela (Uniform)', $student_name, 'Updated uniform record', $created_by);
            } else {
                $error_message = "Error updating record: " . mysqli_error($conn);
            }
        } else {
            $insert_query = "INSERT INTO tblgrade2gumamelauniform (lrn, student_name, gender, pe_tshirt, pe_pants, socks, shoes, school_uniform, uniform_bottom, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                logAction($conn, 'ADD', 'Grade 2 - Gumamela (Uniform)', $student_name, 'Added new uniform record', $created_by);
            } else {
        ...
        $existing_lrns = [];
        $existing_result2 = mysqli_query($conn, "SELECT lrn FROM tblgrade2gumamelauniform");
...
if(isset($_GET['delete']) && $has_access) {
    ...
    $get_name_query = "SELECT student_name FROM tblgrade2gumamelauniform WHERE id = ?";
    ...
    $delete_query = "DELETE FROM tblgrade2gumamelauniform WHERE id = ?";
    $stmt = mysqli_prepare($conn, $delete_query);
    mysqli_stmt_bind_param($stmt, "i", $id);
    if(mysqli_stmt_execute($stmt)) {
        $success_message = "Record deleted successfully!";
        logAction($conn, 'DELETE', 'Grade 2 - Gumamela (Uniform)', $student_name, 'Deleted uniform record', $_SESSION['username']);
        // Refresh existing LRNs after delete
        $existing_lrns = [];
        $existing_result3 = mysqli_query($conn, "SELECT lrn FROM tblgrade2gumamelauniform");
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $query = "SELECT * FROM tblgrade2gumamelauniform WHERE lrn LIKE '%$search%' OR student_name LIKE '%$search%' ORDER BY student_name";
} else {
    $query = "SELECT * FROM tblgrade2gumamelauniform ORDER BY student_name";
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="action-buttons-top no-print">
            ...
            <button class="supplies-btn" onclick="location.href='gumamela_supplies.php'">
        ...
        <div class="search-section no-print">
            <form method="GET" action="">
                <div class="search-container">
                    ...
                    <?php if(!empty($search)): ?>
                        <button type="button" class="btn-clear" onclick="location.href='gumamela_uniform.php'">
